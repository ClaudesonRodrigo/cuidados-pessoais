...
import {
  ...
  getAllUsers, getUpcomingAppointments, updateAppointmentStatus,
  PageData, LinkData, UserData, CouponData, findUserByEmail, updateUserPlan, AppointmentData
...
export default function DashboardPage() {
  ...
  // Load agenda when tab changes to agenda
  ...
  const handleSaveProfile = async () => {
      if(!pageSlug) return;
      if (!isProPlan && (editingProfilePix !== (pageData as any)?.pixKey || editingProfileAddress !== pageData?.address)) {
      }
    ...
    <div className="min-h-screen bg-gray-50 pb-20 font-sans">
      ...
      <main className="max-w-4xl mx-auto py-6 px-4 space-y-6">
        ...
        {daysLeft !== null && daysLeft <= 3 && (
            <div onClick={() => setIsUpgradeModalOpen(true)} className="bg-red-100 border-red-300 text-red-800 p-3 rounded-lg text-sm font-bold flex justify-between cursor-pointer hover:bg-red-200">
                <span>{daysLeft === 0 ? "Teste expirou! Assine agora." : `Teste acaba em ${daysLeft} dias.`}</span>
                <span>Assinar Pro &rarr;</span>
            </div>
        )}
