...
export default function DashboardPage() {
  ...
  // --- CONFIGURAÇÃO DE HORÁRIOS E DIAS (NOVO) ---
    ...
    if (idToFetch) {
      ...
      if (result) {
        ...
        if (data.schedule) {
            ...
            // Se já tiver dias salvos, usa. Se não, padrão Seg-Sab.
  ...
  const handleUpdateItem = async (index: number) => {
    ...
    updated[index] = { 
        ...
        durationMinutes: parseInt(editItemDuration)||30 
    };
        ...
        {activeTab === 'agenda' && (
            <div className="space-y-6">
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 min-h-[400px]">
                    ...
                    {isLoadingAppointments ? <div className="text-center py-10">Carregando...</div> : <div className="space-y-4">{appointments.map(app => {
                        ...
                        return (
                            <div key={app.id} className="border p-4 rounded-xl flex flex-col gap-2 bg-gray-50">
                                ...
                                {app.status === 'pending' && <button onClick={() => handleStatusChange(app.id!, 'confirmed')} className="bg-green-600 text-white p-2 rounded text-xs font-bold">Confirmar Pagamento</button>}
                                {app.status === 'confirmed' && <button onClick={() => handleStatusChange(app.id!, 'completed')} className="bg-blue-600 text-white p-2 rounded text-xs font-bold">Concluir</button>}
                                {app.status !== 'cancelled' && <button onClick={() => handleStatusChange(app.id!, 'cancelled')} className="text-red-500 text-xs hover:underline mt-1 self-start">Cancelar</button>}
                            </div>
                        )
                    })}</div>}
                </div>
